...
(define (sroot value)
  ((iterative-improve
    (lambda (x) (< (abs (- (* x x) value)) 0.0001))
    (lambda (g) (/ (+ g (/ value g)) 2))) 1.0))

(sroot 4)

(define (fixedPointref func guess precision)
  (let ((fixedVal (func guess)))
    (cond ((< (abs (- fixedVal guess)) precision) guess)
	  (else (fixedPointref func fixedVal precision)))))

;;1.46

(define (fixedPointIter func guess precision)
  ((iterative-improve
    (lambda (x) (< (abs (- (func x) x)) 0.0001))
    (lambda (g) (func g))) 1.0))
